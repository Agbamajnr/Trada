    ...
    setup(props, ctx) {
        ...
        const getMsg = async (room) => {
            room.roomChats.forEach(async chat => {
                const fetchedMsg = await axios.get('https://trada-server.onrender.com/api/chatting/chat/' + chat);
        ...
        const sendMsg = async () => {
            await fetch('https://trada-server.onrender.com/api/chatting/message/' + roomInfo.value.id, {
                method: 'POST',
